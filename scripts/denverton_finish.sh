...
echo gpio-dnv > "$target/etc/modules-load.d/gpio.conf"
...
cp "$custom_dir"/denverton_micon_shutdown.py "$target/usr/bin/micon_shutdown.py"
cp "$custom_dir"/denverton_micon_startup.py "$target/usr/bin/micon_startup.py"
